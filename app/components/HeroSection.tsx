...
// ─── HeroSection.tsx — ОБНОВЛЕНО ─────────────────────────────────────────────
// ИЗМЕНЕНИЕ: убран prop onCta. Компонент сам вызывает scrollToSection('contact').
// Это позволяет page.tsx быть Server Component.
//
// Diff:
//   - Убрана interface HeroSectionProps { onCta: () => void }
//   - <button onClick={onCta}> → <button onClick={() => scrollToSection('contact')}>
...
export default function HeroSection() {
  ...
  return (
    ...
    >
      ...
      />
      <motion.div aria-hidden="true" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 2, ease: 'easeOut' }} className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-[#c8a96e]/10 blur-[120px] pointer-events-none" />
      <motion.div aria-hidden="true" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 2, ease: 'easeOut', delay: 0.5 }} className="absolute bottom-1/4 right-1/4 w-64 h-64 rounded-full bg-[#c8a96e]/8 blur-[100px] pointer-events-none" />
      <div aria-hidden="true" className="absolute left-8 top-1/4 bottom-1/4 w-px bg-gradient-to-b from-transparent via-[#c8a96e]/40 to-transparent hidden xl:block" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-[116px] pb-16 lg:pt-28 lg:pb-20">
        <div className="max-w-4xl">
          ...
          <div className={cn('flex items-center gap-3 mb-10 transition-all duration-700 delay-200', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6')}>
            <div className="w-8 h-px bg-[#c8a96e]/60 shrink-0" aria-hidden="true" />
            <p className="text-white/50 text-sm tracking-widest uppercase shrink-0" aria-hidden="true">Изготовим</p>
            <FlipText words={PRODUCTS} />
          </div>

          <p className={cn('text-white/60 text-lg text-justify max-w-2xl leading-relaxed mb-12 transition-all duration-700 delay-300', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6')}>
            Создаём эксклюзивные изделия из ценных пород дерева по индивидуальным проектам.
            Профессиональное производство, гарантия качества, доставка и монтаж по всему Санкт-Петербургу.
          </p>

          <div className={cn('flex flex-col sm:flex-row gap-4 transition-all duration-700 delay-[400ms]', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6')}>
            {/* ИЗМЕНЕНО: onClick={() => scrollToSection('contact')} вместо onCta */}
            <button
          ...
          <div className={cn('mt-16 pt-8 border-t border-white/10 grid grid-cols-3 gap-8 max-w-lg transition-all duration-700 delay-500', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6')} aria-label="Ключевые показатели">
            {[
              ...
              <div key={stat.label}>
                <p className="text-3xl font-bold text-[#c8a96e]" aria-label={`${stat.value} ${stat.label}`}>{stat.value}</p>
                <p className="text-white/40 text-xs uppercase tracking-widest mt-1" aria-hidden="true">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <motion.div aria-hidden="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 hidden md:flex flex-col items-center gap-2 text-white/30 pointer-events-none">
